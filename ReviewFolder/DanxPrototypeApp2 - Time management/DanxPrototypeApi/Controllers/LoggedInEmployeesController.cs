...
using DanxPrototypeApi;

namespace DanxPrototypeApi.Controllers
{
    public class LoggedInEmployeesController : ApiController
    {
        private LoggedInEmployeeContext db = new LoggedInEmployeeContext();
        ...
        public IHttpActionResult PostLoggedInEmployee(LoggedInEmployee loggedInEmployee)
        {
            ...
            {
                if (LoggedInEmployeeExists(loggedInEmployee.Id))
                {
                    return Conflict();
                }
                else
            ...
            return CreatedAtRoute("DefaultApi", new { id = loggedInEmployee.Id }, loggedInEmployee);
        }
        ...
        {
            return db.LoggedInEmployees.Count(e => e.Id == id) > 0;
        }
    }
}